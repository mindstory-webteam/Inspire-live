...
export default function OurResourcePerson() {
  ...
  const fetchMembers = async () => {
    ...
    setError("");
    try {
      const res  = await fetch(`${API_BASE}/admin/ourresourceperson`, { headers: getAuthHeaders() });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url    = editId ? `${API_BASE}/admin/ourresourceperson/${editId}` : `${API_BASE}/admin/ourresourceperson`;
  ...
  const confirmDelete = async () => {
    if (!deleteId) return;
    try {
      const res  = await fetch(`${API_BASE}/admin/ourresourceperson/${deleteId}`, {
  ...
  return (
    <div style={s.page}>
      ...
      <div style={s.topBar}>
        <div>
          <h1 style={s.pageTitle}>Our Resource Person</h1>

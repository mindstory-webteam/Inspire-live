...
const getAllContacts = async (req, res) => {
  try {
    const { page = 1, limit = 15, search, status, sortBy = 'createdAt', order = 'desc' } = req.query;
    const filter = {};
    ...
    const [contacts, total] = await Promise.all([
      Contact.find(filter).sort({ [sortBy]: sortOrder }).skip(skip).limit(parseInt(limit)),
      Contact.countDocuments(filter),
    ]);
    res.status(200).json({ success: true, data: contacts, pagination: { total, page: parseInt(page), limit: parseInt(limit), totalPages: Math.ceil(total / parseInt(limit)) } });
  } catch (error) {
...
const getStats = async (req, res) => {
  try {
    ...
    res.status(200).json({ success: true, data: { total, new: newCount, read, replied, archived, last30Days } });
  } catch (error) {
    res.status(500).json({ success: false, message: 'Server error', error: error.message });
  }
};

...
const getCareerById = async (req, res) => {
  try {
    const career = await Career.findOne({ _id: req.params.id, isActive: true }).select(
      '-applications'
    );
  ...
  try {
    ...
    const career = await Career.create({
      ...
      requirementsList: Array.isArray(requirementsList)
        ? requirementsList
        : requirementsList
        ? JSON.parse(requirementsList)
        : [],
      responsibilities,
      responsibilitiesList: Array.isArray(responsibilitiesList)
        ? responsibilitiesList
        : responsibilitiesList
        ? JSON.parse(responsibilitiesList)
        : [],
      jobNumber,
      company,
      website,
...
const updateCareer = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ success: true, message: 'Career updated successfully', data: updatedCareer });
  } catch (error) {
